...
function applyRegistrationPlatformField(reg=null){
  ...
  // Platform is already represented by the linked reservation and platform badges.
  // Only show the editable field for manual/unlinked guest registrations.
  $('regPlatformField').classList.toggle('hidden',linked);
}
...
function renderRegs(){
  ...
  rows.forEach(r=>{
    ...
    const platform=effectiveRegistrationPlatform(r), platformBadge=platform?`<span class="badge platform-archive ${platform==='airbnb'?'airbnb':platform==='booking'?'booking':''}">${escapeHtml(platformDisplay(platform))}</span>`:'';
    const d=document.createElement('div'); d.className='registration-item list-item-with-action';
    d.innerHTML=`<div class="list-item-content"><strong>${escapeHtml(r.full_name)}</strong><span class="muted">${fmt(r.checkin_date)} → ${fmt(r.checkout_date)}</span><div class="registration-badges">${platformBadge}<span class="badge ${r.id_verified?'good':'warn'}">${r.id_verified?(currentLang==='nl'?'ID gecontroleerd':'ID verified'):(currentLang==='nl'?'ID nog controleren':'ID to verify')}</span>${invoiceBadge}<span class="badge">${r.source==='paper_manual'?(currentLang==='nl'?'Papier':'Paper'):(currentLang==='nl'?'Digitaal':'Digital')}</span></div></div><button type="button" class="quick-delete-btn" title="${escapeHtml(x.quickDeleteReg)}" aria-label="${escapeHtml(x.quickDeleteReg)}">🗑</button>`;
