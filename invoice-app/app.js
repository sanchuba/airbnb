...
function setCalendarRoomView(roomKey){calendarRoomView=roomKey;renderCalendar();}
function exitCalendarRoomView(){calendarRoomView=null;renderCalendar();requestAnimationFrame(()=>{$('calendarScroller').scrollLeft=calendarTimelineScrollLeft;});}
function roomMonthGridStart(){
  const first=new Date(calendarCursor.getFullYear(),calendarCursor.getMonth(),1),offset=(first.getDay()+6)%7,d=new Date(first);d.setDate(first.getDate()-offset);return d;
}
function renderRoomMonthCalendar(roomKey){
  ...
  const weekdays=[],monday=new Date(2026,0,5);
  for(let i=0;i<7;i++){const d=new Date(monday);d.setDate(d.getDate()+i);weekdays.push(new Intl.DateTimeFormat(locale,{weekday:'short'}).format(d).replace('.',''));}
  const wr=document.createElement('div');wr.className='room-month-weekdays';wr.innerHTML=weekdays.map(w=>`<span>${escapeHtml(w)}</span>`).join('');root.appendChild(wr);

  const gridStart=roomMonthGridStart(),monthStart=monthStartIso(),monthEnd=nextMonthStartIso();
  const offset=(new Date(calendarCursor.getFullYear(),calendarCursor.getMonth(),1).getDay()+6)%7,weeks=Math.ceil((offset+calendarDaysInMonth())/7);
  const events=[...reservations.filter(isRealReservation),...calendarVisibleBlocks()].filter(r=>r.room_key===roomKey&&r.checkin_date<monthEnd&&r.checkout_date>monthStart);
  ...
  for(let w=0;w<weeks;w++){
    ...
    for(let i=0;i<7;i++){
      const d=new Date(wsDate);d.setDate(wsDate.getDate()+i);const iso=isoLocalDate(d),inMonth=d.getMonth()===calendarCursor.getMonth();
      const cell=document.createElement('div');cell.className=`room-month-day ${inMonth?'':'outside'} ${iso===today?'today':''} ${iso<today?'past':''}`;
      cell.innerHTML=`<span class="room-month-day-number">${d.getDate()}</span>`;week.appendChild(cell);
    }
    const layer=document.createElement('div');layer.className='room-month-event-layer';
    events.filter(r=>r.checkin_date<we&&r.checkout_date>ws).forEach(r=>{
      const segStart=r.checkin_date<ws?ws:r.checkin_date,segEnd=r.checkout_date>we?we:r.checkout_date;
      const diff=x=>Math.round((new Date(x+'T00:00:00')-new Date(ws+'T00:00:00'))/86400000);
      const s=Math.max(0,diff(segStart)),e=Math.max(s+1,diff(segEnd));
      const bar=document.createElement('button');bar.type='button';bar.className=`room-month-event ${isAirbnbBlock(r)?'blocked':r.platform}`;bar.style.gridColumn=`${s+1} / ${Math.min(8,e+1)}`;
    ...
    week.appendChild(layer);root.appendChild(week);
  }
}
function renderCalendar(){
  ...
  rooms.forEach(([roomKey,label])=>{
    ...
    events.filter(r=>r.room_key===roomKey&&r.checkin_date<end&&r.checkout_date>start).sort((a,b)=>a.checkin_date.localeCompare(b.checkin_date)).forEach(r=>{
      ...
      bar.style.left=`calc(${startDay} * (100% / ${days}) + ${r.checkin_date>=start?'2px':'0px'})`;bar.style.width=`calc(${endDay-startDay} * (100% / ${days}) - ${r.checkout_date<=end?'4px':'0px'})`;
...
function calendarToday(){const n=new Date();calendarCursor=new Date(n.getFullYear(),n.getMonth(),1);renderCalendar();if(!calendarRoomView)scrollCalendarToTodayStart('smooth');}
