...
function clearOvernightEditor(){
  ...
  $('overnightPaid').checked=false;$('overnightIdVerified').checked=false;
...
function openOvernightEditor(g=null){
  ...
  if(!g){ clearOvernightEditor(); }
    ...
    $('overnightPaid').checked=!!g.fee_paid;$('overnightIdVerified').checked=!!g.id_verified;
...
function renderOvernightGuests(){
  ...
  rows.forEach(g=>{
    ...
    const idBadge=g.id_verified?`<span class="badge good">${currentLang==='nl'?'ID gecontroleerd':'ID checked'}</span>`:''; const paid=g.fee_paid?`<span class="badge good">${currentLang==='nl'?'Betaald':'Paid'}</span>`:`<span class="badge warn">${currentLang==='nl'?'Nog te betalen':'Payment due'}</span>`;
    d.innerHTML=`<span><strong>${escapeHtml(g.full_name||((currentLang==='nl')?'Registratie nog niet ingevuld':'Registration not completed'))}</strong><small>${fmt(g.checkin_date)} → ${fmt(g.checkout_date)} · ${overnightNights(g)} ${escapeHtml(tr[currentLang].nightsWord)}</small></span><span>${idBadge}${paid}</span>`;
