...
function bookingReferenceForReservation(r){
  ...
  const fromReg=normalizeBookingReservationId(reg?.booking_reference);
  if(fromReg)return fromReg;
  const inv=latestReservationInvite(r.id);
  const fromInvite=normalizeBookingReservationId(inv?.booking_reference);
