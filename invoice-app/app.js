...
};$('newPaperBtn').onclick=blankRegistration;$('closeRegistrationBtn').onclick=()=>{if(!guardUnsaved('registration'))return;registrationDirty=false;currentDraftReservationId=null;updateMobileSaveBar();$('registrationEditor').classList.add('hidden');};$('saveRegistrationBtn').onclick=async()=>{await saveReg();updateMobileSaveBar();};
...
document.querySelectorAll('[data-reservation-filter]').forEach(b=>b.onclick=()=>setReservationFilter(b.dataset.reservationFilter));
document.querySelectorAll('.lang-btn').forEach(b=>b.onclick=()=>{currentLang=b.dataset.lang;setTexts();toggleRegInvoice();toggleIdOther();toggleInvoiceCustom();renderReservations();updateMobileSaveBar();renderCalendarSyncAge();if(pendingLoginEmail)startOtpCooldown(pendingLoginEmail);});
