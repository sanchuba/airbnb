...
function buildInvoicePaymentText(registrationId){
  ...
  let extraText='';
  if(method==='airbnb') extraText=`${currentLang==='nl'?'Extra overnachtende gast':'Additional overnight guest'}: ${tr[currentLang].airbnb}`;
  if(method==='paypal') extraText=`${currentLang==='nl'?'Extra overnachtende gast':'Additional overnight guest'}: ${currentLang==='nl'?'Betaald via PayPal':'Paid via PayPal'}`;
  if(method==='cash') extraText=`${currentLang==='nl'?'Extra overnachtende gast':'Additional overnight guest'}: ${currentLang==='nl'?'Contant betaald':'Paid in cash'}`;
...
function useRegistrationForInvoice(){
  ...
  manualNights=false;autoNights();$('deleteBtn').classList.add('hidden');$('duplicateBtn').classList.add('hidden');updatePreview();markInvoiceDirty();suppressDirty=false;document.querySelector('.form-card').scrollIntoView({behavior:'smooth'});
}
...
$('regAdditionalGuestPaid').onchange=()=>{markRegistrationDirty();};$('useForInvoiceBtn').onclick=useRegistrationForInvoice;$('deleteRegistrationBtn').onclick=deleteRegistration;$('registrationSearch').oninput=renderRegs;rf.invoiceRequested.onchange=toggleRegInvoice;rf.invoiceType.onchange=toggleRegInvoice;rf.idType.onchange=toggleIdOther;
...
document.querySelectorAll('[data-reservation-filter]').forEach(b=>b.onclick=()=>setReservationFilter(b.dataset.reservationFilter));
document.querySelectorAll('.lang-btn').forEach(b=>b.onclick=()=>{currentLang=b.dataset.lang;setTexts();toggleRegInvoice();toggleIdOther();toggleInvoiceCustom();renderReservations();updateMobileSaveBar();});
