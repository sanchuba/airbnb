...
let currentLang='en', currentRegistrationId=null, currentInvoiceId=null, registrations=[], invoices=[], manualNights=false, registrationFilter='all', registrationDirty=false, invoiceDirty=false, suppressDirty=false;
...
function setRegistrationFilter(filter){ registrationFilter=filter; document.querySelectorAll('.filter-pill').forEach(b=>b.classList.toggle('active',b.dataset.filter===filter)); renderRegs(); }
...
async function loadRegs(){const {data,error}=await supabaseClient.from('guest_registrations').select('*').order('checkin_date',{ascending:false}).limit(250);if(error){$('registrationList').innerHTML=`<p class="muted">${error.message}</p>`;return;}registrations=data||[];renderAttention();renderRegs();}
...
function renderRegs(){
  ...
  const rows=registrations.filter(r=>registrationMatchesFilter(r,registrationFilter)).filter(r=>[r.full_name,r.email,r.booking_reference,r.company_name,r.city].filter(Boolean).some(v=>String(v).toLowerCase().includes(q)));
  ...
  rows.forEach(r=>{
    ...
    const badge=d.querySelector('.linked-invoice-badge'); if(badge) badge.onclick=e=>{e.stopPropagation();loadInvoice(linked);};
...
async function loadInvoices(){const {data,error}=await supabaseClient.from('invoices').select('*').order('invoice_year',{ascending:false}).order('invoice_sequence',{ascending:false}).limit(300);if(error){$('invoiceList').innerHTML=`<p class="muted">${error.message}</p>`;return;}invoices=data||[];renderInvoices();renderAttention();renderRegs();}
...
document.querySelectorAll('.attention-card').forEach(b=>b.onclick=()=>{setRegistrationFilter(b.dataset.filter);$('registrationOverview').scrollIntoView({behavior:'smooth',block:'end'});});
...
document.querySelectorAll('.lang-btn').forEach(b=>b.onclick=()=>{currentLang=b.dataset.lang;setTexts();toggleRegInvoice();toggleIdOther();toggleInvoiceCustom();});
