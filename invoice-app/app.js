...
function calendarEventName(r){
  ...
  if(r.platform==='booking'){
    ...
    return bookingId||tr[currentLang].calendarBookingReservation;
  }
...
function reservationMatchesFilter(r,filter){
  ...
  if(filter==='missingBookingReference') return r.status==='active' && r.platform==='booking' && r.checkout_date>t && !bookingReferenceForReservation(r);
  if(filter==='expiredRegistrationLink') return r.status==='active' && r.checkout_date>t && !reg && !reservationInvite(r.id) && !!expiredReservationInvite(r.id);
...
function renderAttention(){
  ...
  const missingBookingRef=active.filter(r=>r.platform==='booking' && r.checkout_date>t && !bookingReferenceForReservation(r)).length;
  const expiredLinks=active.filter(r=>r.checkout_date>t && !reservationRegistration(r.id) && !reservationInvite(r.id) && !!expiredReservationInvite(r.id)).length;
  const values={arrivingSoon:arriving,idToVerify:ids,invoiceToCreate:inv,missingBookingReference:missingBookingRef,expiredRegistrationLink:expiredLinks};
  ...
  const any=arriving+ids+inv+missingBookingRef+expiredLinks>0;
